...
public class AccountController extends HelloUI {
    ...
    @FXML private void accountAction() {
        ...
        if(form.isValid()) {
            ...
            if(name.getText().length() == 0) user.setName(null);
            else user.setName(name.getText());
            if(surname.getText().length() == 0) user.setSurname(null);
            else user.setSurname(surname.getText());
            if(phone.getText().length() == 0) user.setPhone(null);
            else user.setPhone(phone.getText());
            if(email.getText().length() == 0) user.setEmail(null);
            else user.setEmail(email.getText());
            if(organisation.getText().length() == 0) user.setOrganisation(null);
            else user.setOrganisation(organisation.getText());
            if(job.getText().length() == 0) user.setJob(null);
